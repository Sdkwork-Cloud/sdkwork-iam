...
- Session creation, current session, logout, refresh, OAuth, QR auth, password reset, verification code, and runtime metadata always use `appbaseAppClient`, which must be the generated `@sdkwork/appbase-app-sdk` client or an approved adapter over it.
- Administrative IAM resources use `appbaseBackendClient` from `@sdkwork/appbase-backend-sdk` when provided.
