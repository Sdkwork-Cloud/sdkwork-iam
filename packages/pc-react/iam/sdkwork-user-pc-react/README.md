...
- Profile update and password update must be added to `sdkwork-appbase-app-api` as semantic current-user resources, exposed by generated app SDK methods such as `appbaseApp.iam.users.current.update(...)` and `appbaseApp.iam.users.current.password.update(...)`, before applications use those capabilities.
