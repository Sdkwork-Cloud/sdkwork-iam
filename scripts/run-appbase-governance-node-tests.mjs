...
import { listAppbaseGovernanceNodeTestFiles } from "./appbase-governance-node-test-catalog.mjs";
...
export function listAppbaseGovernanceNodeTests() {
  return listAppbaseGovernanceNodeTestFiles();
}

export function createAppbaseGovernanceNodeTestPlan({
  cwd = appbaseRoot,
  env = process.env,
  nodeExecutable = process.execPath,
  platform = process.platform,
  testFiles = listAppbaseGovernanceNodeTests(),
} = {}) {
  return {
...
function buildCommandFailure(plan, result) {
  ...
  return new Error(
    `appbase governance node tests failed with exit code ${result?.status ?? "unknown"} while executing ${plan.command} ${plan.args.join(" ")}${fragments.length > 0 ? `\n${fragments.join("\n")}` : ""}`,
  );
}

export function runAppbaseGovernanceNodeTests({
  cwd = appbaseRoot,
  env = process.env,
  nodeExecutable = process.execPath,
  platform = process.platform,
  ...
  const plan = createAppbaseGovernanceNodeTestPlan({
    cwd,
    env,
    nodeExecutable,
    platform,
