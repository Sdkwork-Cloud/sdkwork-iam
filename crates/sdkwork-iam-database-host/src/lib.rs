...
pub use iam_database_module::AppbaseIamDatabaseModule;
...
pub struct IamDatabaseHost {
    pool: DatabasePool,
    module: Arc<AppbaseIamDatabaseModule>,
}
...
impl IamDatabaseHost {
    ...
    pub fn module(&self) -> Arc<AppbaseIamDatabaseModule> {
        self.module.clone()
    }
...
pub async fn bootstrap_iam_database(pool: DatabasePool) -> Result<IamDatabaseHost, String> {
    ...
    let module = Arc::new(
        AppbaseIamDatabaseModule::from_app_root(&app_root)
    ...
    let orchestrator =
        LifecycleOrchestrator::new(pool.clone(), module.clone()).with_applied_by("sdkwork-appbase");
