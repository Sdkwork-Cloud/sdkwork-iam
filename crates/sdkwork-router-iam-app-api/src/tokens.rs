...
async fn ensure_tenant_signing_key(
    ...
    if let Some(row) = row {
        ...
        let secret = sdkwork_iam_web_adapter::decode_signing_secret_ref(&secret_ref)?;
        return Ok(LocalTenantSigningKey {
            tenant_id,
            kid,
            secret,
        });
    ...
    .execute(pg)
    .await
    .map_err(|e| format!("insert signing key failed: {e}"))?;

    // If another request concurrently inserted a key, load theirs
    let row = sqlx::query(
        "SELECT tenant_id, kid, secret_ref FROM iam_tenant_signing_key \
         WHERE tenant_id = $1 AND status = 'active' \
         ORDER BY active_from DESC LIMIT 1",
    ...
    .fetch_optional(pg)
    .await
    .map_err(|e| format!("load signing key after insert failed: {e}"))?
...
async fn load_signing_key_by_kid(pg: &PgPool, kid: &str) -> Option<LocalTenantSigningKey> {
    ...
    let kid: String = row.get(1);
    let secret_ref: String = row.get(2);
    let secret = sdkwork_iam_web_adapter::decode_signing_secret_ref(&secret_ref).ok()?;
    Some(LocalTenantSigningKey {
        tenant_id,
        kid,
        secret,
    })
