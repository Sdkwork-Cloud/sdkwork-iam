...
use crate::{
    directory::SdkworkIamLocalIamDirectory, handlers,
    password_session_bridge::PasswordSessionBridge, state::LocalIamState,
};

pub async fn build_sdkwork_iam_app_api_router() -> Result<Router, String> {
    handlers::build_sdkwork_iam_app_api_router().await
}

pub async fn build_sdkwork_iam_app_api_router_with_web_resolver<R>(
    resolver: R,
) -> Result<Router, String>
where
    R: sdkwork_web_core::WebRequestContextResolver + Clone + Send + Sync + 'static,
{
    handlers::build_sdkwork_iam_app_api_router_with_web_resolver(resolver).await
}

pub async fn build_sdkwork_iam_app_api_router_with_local_directory(
) -> Result<(Router, SdkworkIamLocalIamDirectory), String> {
    handlers::build_sdkwork_iam_app_api_router_with_local_directory().await
}

pub async fn build_sdkwork_iam_app_api_router_with_pool(
...
pub async fn build_sdkwork_iam_oauth_device_authorization_router_with_pool_and_password_session_bridge(
    pool: DatabasePool,
