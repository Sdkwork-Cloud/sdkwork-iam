...
```bash
cargo add sdkwork-iam-backend-sdk
```

## Quick Start

```rust
use sdkwork_iam_backend_sdk::{SdkworkBackendClient, SdkworkConfig};
...
client.set_access_token("your-access-token");
    ...
    let result = client.iam().account_binding_policy_retrieve().await?;
    println!("{result:?}");
    Ok(())
}
```
...
let outcome: Result<(), _> = async {
    client.iam().account_binding_policy_retrieve().await?;
    Ok(())
}.await;
