...
```swift
dependencies: [
    .package(url: "https://github.com/sdkwork/sdkwork-iam-backend-sdk", from: "1.0.0")
]
```
...
// Use the SDK
let result = try await client.iam.accountBindingPolicyRetrieve()
print(result)
```
...
```swift
do {
    try await client.iam.accountBindingPolicyRetrieve()
} catch {
    print("Error: \(error)")
}
```
