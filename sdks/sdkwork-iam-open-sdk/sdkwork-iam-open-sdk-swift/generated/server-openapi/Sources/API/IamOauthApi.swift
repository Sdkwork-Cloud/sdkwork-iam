...
public class IamOauthApi {
    ...
    public func providerCallbacksHandleGet(callbackPublicId: String) async throws -> AppbaseApiResult? {
        return try await client.request("GET", ApiPaths.customPath("/oauth/provider_callbacks/\(serializePathParameter(callbackPublicId, PathParameterSpec(name: "callbackPublicId", style: "simple", explode: false)))"), body: nil, params: nil, headers: nil, skipAuth: true, responseType: AppbaseApiResult.self)
    }

    /// Iam oauth provider Callbacks handle Post.
    public func providerCallbacksHandlePost(callbackPublicId: String, body: [String: Any]) async throws -> AppbaseApiResult? {
        return try await client.request("POST", ApiPaths.customPath("/oauth/provider_callbacks/\(serializePathParameter(callbackPublicId, PathParameterSpec(name: "callbackPublicId", style: "simple", explode: false)))"), body: body, params: nil, headers: nil, contentType: "application/json", skipAuth: true, responseType: AppbaseApiResult.self)
    }
