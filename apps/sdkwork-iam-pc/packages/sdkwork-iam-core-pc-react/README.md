...
The adapter must not map legacy methods such as `auth.login`, `auth.refreshToken`, `auth.register`, or `user.getUserProfile` into appbase login ports. Fix `sdkwork-iam-app-api`, OpenAPI, and SDK generation instead.

`appbaseAppClient` owns login, session creation, OAuth, password reset, verification code, and current-user self-service APIs. It must be the generated `@sdkwork/iam-app-sdk` client or a strict adapter over that generated appbase app SDK resource surface. `appbaseBackendClient` owns administration and IAM management resources through `@sdkwork/iam-backend-sdk`. Backend management calls do not fall back to `appbaseAppClient.iam`; this keeps app API and backend API boundaries explicit.
...
```bash
pnpm run test:workspace-vitest run apps/sdkwork-iam-pc/packages/sdkwork-iam-core-pc-react/tests/iam-core.service.test.ts
```
