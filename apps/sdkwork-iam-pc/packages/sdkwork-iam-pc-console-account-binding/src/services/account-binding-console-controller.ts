...
import type {
  ...
  CreateSdkworkIamConsoleAccountBindingControllerInput,

  SdkworkIamConsoleAccountBindingController,

  SdkworkIamConsoleAccountBindingPolicy,

  SdkworkIamConsoleAccountBindingState,

  SdkworkIamConsoleOAuthAccountLink,

} from "../types/account-binding-console-types";

import { DEFAULT_CONSOLE_ACCOUNT_BINDING_POLICY as defaultPolicy } from "../types/account-binding-console-types";



export function createSdkworkIamConsoleAccountBindingController(

  input: SdkworkIamService | CreateSdkworkIamConsoleAccountBindingControllerInput,

): SdkworkIamConsoleAccountBindingController {

  const service = "service" in input ? input.service : input;

  let state: SdkworkIamConsoleAccountBindingState = { accountLinks: [], status: "idle" };
  ...
  const setState = (patch: Partial<SdkworkIamConsoleAccountBindingState>) => {
  ...
  const controller: SdkworkIamConsoleAccountBindingController = {
    ...
    listAccountLinks: async (params) => {
      ...
      try {
        ...
        const accountLinks = (await accountLinksSession.list(params)) as SdkworkIamConsoleOAuthAccountLink[];
    ...
    loadMoreAccountLinks: async () => {
      ...
      try {
        ...
        const accountLinks = (await accountLinksSession.loadMore()) as SdkworkIamConsoleOAuthAccountLink[];
    ...
    refreshWorkspace: async (params) => {
      ...
      const [policy, accountLinks] = await Promise.all([
        ...
        controller.listAccountLinks(params),

      ]);

      return { accountLinks, policy };
...
function normalizePolicy(value: unknown): SdkworkIamConsoleAccountBindingPolicy {
...
function toAccountLink(value: unknown): SdkworkIamConsoleOAuthAccountLink | undefined {
