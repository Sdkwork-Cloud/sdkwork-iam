...
/**
 * Unwrap tree root nodes from `SdkWorkResourceResponse` (`data.item.nodes`) or SDK-unwrapped `{ nodes }`.
 */
export function extractSdkWorkTreeNodes<T = unknown>(value: unknown): readonly T[] {
  if (Array.isArray(value)) {
    return value as readonly T[];
  }
  if (!isRecord(value)) {
  ...
  if (Array.isArray(value.nodes)) {
    return value.nodes as readonly T[];
  }
  if (isRecord(value.item) && Array.isArray(value.item.nodes)) {
    return value.item.nodes as readonly T[];
  }
  if (isRecord(value.data) && isRecord(value.data.item) && Array.isArray(value.data.item.nodes)) {
    return value.data.item.nodes as readonly T[];
  }
  return [];
}
...
/** Default offset list query for IAM admin/console surfaces. */
 ...
 * Preserves domain filters (for example `organizationId`) while enforcing `page_size`.
 */
...
/** Mutable server-backed list session for IAM admin/console controllers. */
