...
Applications should pass the generated `@sdkwork/iam-app-sdk` client as `appbaseApp` and the generated `@sdkwork/iam-backend-sdk` client as `appbaseBackend`. Business modules should depend on the standard IAM ports, not on app-specific generated SDK constructors or method aliases.

Backend OAuth management routes (`iamOauth.iam.oauth.*`) are adapted recursively via `backend-oauth-resource-tree.ts` into `service.iam.oauth.*` ports consumed by `@sdkwork/iam-service`.
